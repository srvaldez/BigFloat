...
*/
void DecFloat_Set_Digits(int32_t digits, int32_t guard_words) {
	int32_t  n = digits / 8;
	if ((8 * n) == digits) {
...
void pi_chudnovsky_binary_split(uint64_t a, uint64_t b, DecFloat_struct* pab, DecFloat_struct* qab, DecFloat_struct* tb, int32_t dwords) {
	...
	DecFloat_init(&c545140134);
	DecFloat_init(&c13591409);
	DecFloat_init(&temp);
	DecFloat_init(&c3_over_24);
...
void fpatn(DecFloat_struct* result, DecFloat_struct* x, int32_t dwords) {
	...
	DecFloat_init(&mt2);
	DecFloat_init(&p);
	DecFloat_init(&bignumber);
	DecFloat_init(&bignumber2);
	DecFloat_init(&one);
	DecFloat_init(&factor);
	DecFloat_init(&swap);
