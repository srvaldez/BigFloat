...
'<<< must use redim for shared arrays to avoid heap corruption
reDim Shared As Decfloat roots(order), weights(order), legcoef(order, order)
Dim As Double tm, t
...
Sub legroots
	...
	eps="1.0e-"+str(digits_precision-5)
